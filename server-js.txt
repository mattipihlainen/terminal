...
function displayCalendarEvents(events) {
  const filtered = events.filter((event) => !/CLARE\s+WORK/i.test(event.title || ""));
  const seen = new Set();
  const unique = [];

  for (const event of filtered) {
    const displayTime = event.end_time ? `${event.time}–${event.end_time}` : event.time;
    const key = [
      String(event.title || "").trim().toLowerCase(),
      String(displayTime || "").trim().toLowerCase(),
...
function buildClareWorkSummary(events, rows, today, tomorrow, nowHour) {
  ...
  const active = present && nowHour >= 16 && nowHour < 19;
  ...
  if (!active) {
    return {
      ...
      event_time: event ? (event.end_time ? `${event.time}–${event.end_time}` : event.time) : ""
    };
  }
...
async function makePayload() {
  ...
  const payload = {
    ...
    calendar: {
      ...
      empty_message: ICLOUD_CALENDAR_URLS.length > 0
        ? (clareWork && clareWork.present ? "No other calendar events today" : "Nothing on these calendars today")
